    ...
    with tab2:
        ...
        if st.button("🚀 สร้างไฟล์ใบส่งสินค้า (ฉบับพิมพ์ต่อเนื่อง)"):
            ...
            with pd.ExcelWriter(output_do, engine='xlsxwriter') as writer:
                ...
                # สไตล์ Ship To (B:C Merge) และ Delivery Date
                ...
                # --- [2. ตั้งค่าหน้ากระดาษ A4] ---
                worksheet.set_paper(9) # A4
                worksheet.set_margins(0.3, 0.3, 0.3, 0.3)
                worksheet.set_column('A:A', 5)   # No.
                worksheet.set_column('B:B', 14)  # Product Code
                worksheet.set_column('C:C', 45)  # Name (แกนกลาง)
                worksheet.set_column('D:D', 12)  # Unit
                worksheet.set_column('E:E', 15)  # QTY
                
                curr = 0 # ตัวนับแถว (Row Cursor)
                page_breaks = [] # เก็บตำแหน่งที่จะสั่งขึ้นหน้าใหม่

                for store_code in df_clean['รหัสสาขา'].unique():
                    df_store = df_clean[df_clean['รหัสสาขา'] == store_code].copy()
                    first_row = df_store.iloc[0]

                    # --- [3. ตรวจสอบพื้นที่ก่อนเริ่ม DO ใหม่] ---
                    # ถ้าแถวปัจจุบันเกินบรรทัดที่ 35 (โดยประมาณ) ให้สั่งขึ้นหน้าใหม่
                    if curr > 35:
                        page_breaks.append(curr)
                        # ในระบบ Print Continuous ของโปรเจกต์เดิม เราจะไม่ reset curr 
                        # แต่จะให้มันรันต่อไปเรื่อยๆ แล้วใช้ Page Break เป็นตัวตัดหน้า

                    # --- [4. เขียนข้อมูลส่วนหัว (Header)] ---
                    ...
                    # ตำแหน่ง C2 (Title) ตามที่คุณต้องการเป๊ะๆ
                    ...
                    # Delivery Date (ล็อคตำแหน่ง D8:E8 ของแต่ละ DO)
                    ...
                    # Merge B:C สำหรับที่อยู่ และไม่มีกรอบ (No Stroke) ตามสั่ง
                    ...
                    f_ptr = row_ptr + 1
                    worksheet.set_row(f_ptr + 1, 60) # ความสูงช่องเซ็นชื่อ
                    ...
                    worksheet.merge_range(f_ptr+1, 2, f_ptr+1, 3, 'ชื่อ:\nวันที่:', f_border)
                    
                    worksheet.write(f_ptr, 4, 'คลังสินค้า', f_table_head)
                    worksheet.write(f_row+1, 4, 'ชื่อ:\nวันที่:', f_border)

                    # เลื่อน Cursor ไปยังจุดเริ่มต้นของ DO ถัดไป (เว้น 4 แถว)
                    curr = f_ptr + 4
            ...
            st.success("✅ เรียบร้อย! รวม Sheet เดียว แยกหน้าพิมพ์เป๊ะตามแบบโปรเจกต์เดิมแล้วครับ")
            st.download_button("📥 โหลดไฟล์ DO Master", output_do.getvalue(), "DO_Master_Final.xlsx")