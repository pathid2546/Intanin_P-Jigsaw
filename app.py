...
st.markdown("""<div class="main-header"><h1 style='text-align: center;'>🚛 Intanin Receipt Convert</h1><p style='text-align: center; color: #666;'>Official Logistics Document System</p></div>""", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    tab1, tab2 = st.tabs(["✂️ 1. แยกซัพพลายเออร์ (Logic V12.5)", "📄 2. ออกใบส่งสินค้า (Logic V12.5)"])

    # --- TAB 1: Logic V12.5 เป๊ะๆ ---
    with tab1:
        ...
        if submit_split:
            ...
            with pd.ExcelWriter(output_split, engine='xlsxwriter') as writer:
                ...
                for supplier, sheet_name in current_mapping.items():
                    ...
                    # คอลัมน์กว้างเท่ากัน (ตาม V12.5)
                    worksheet.set_column('A:B', 15); worksheet.set_column('C:C', 15); worksheet.set_column('D:D', 35); worksheet.set_column('E:G', 15)
                    worksheet.set_column('J:J', 15); worksheet.set_column('K:K', 15); worksheet.set_column('L:L', 35); worksheet.set_column('M:M', 15)

            st.success("✅ ประมวลผลสำเร็จ!")
            st.download_button(label="📥 ดาวน์โหลดไฟล์ Splitter", data=output_split.getvalue(), file_name="Intanin_Splitter_V12.6.xlsx")

    # --- TAB 2: Logic DO Master V12.5 เป๊ะๆ ---
    with tab2:
        df_clean = df_raw.dropna(subset=['รหัสสาขา']).copy()
        if st.button("🚀 สร้างใบส่งสินค้า (Official)"):
            ...
            with pd.ExcelWriter(output_do, engine='xlsxwriter') as writer:
                ...
                for store_code in df_clean['รหัสสาขา'].unique():
                    ...
                    worksheet.write(curr+5, 0, f'Store Code: {store_code}', f_bold); worksheet.write(curr+6, 0, f'Store Name: {first["Store Name"]}', f_bold); worksheet.write(curr+7, 0, f'Ship To: {first["ที่อยู่"]}', f_std)
                    ...
                    worksheet.merge_range(curr+7, 3, curr+8, 3, "Delivery\nDate", f_deliv); worksheet.merge_range(curr+7, 4, curr+8, 4, delivery, f_deliv)
                    t_h = curr + 10 
            ...
            st.download_button("📥 ดาวน์โหลด DO Master", output_do.getvalue(), "Intanin_DO_Final.xlsx")
