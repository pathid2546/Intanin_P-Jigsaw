...
# --- [SECURITY] เพิ่มไลบรารีสำหรับระบบยืนยันตัวตนองค์กร ---
...
# ========================================================
# 🔒 [SECURITY CENTER] ระบบล็อกอินด้วย EMAIL องค์กร (Google OAuth 2.0)
# ========================================================

# 1. ดึงข้อมูล Credentials ความปลอดภัยที่ตั้งค่าไว้ใน .streamlit/secrets.toml
# (เมื่อขึ้นระบบจริง แนะนำให้เอาไปใส่ในไฟล์ secrets หรือ Environment Variables เพื่อความปลอดภัยสูงสุด)
GOOGLE_CLIENT_ID = st.secrets.get("google", {}).get("client_id", "241894733535-ob162eaflksj2f8dgqlk1bmo11f8kqeu.apps.googleusercontent.com")
GOOGLE_CLIENT_SECRET = st.secrets.get("google", {}).get("client_secret", "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")
...
# ฟังก์ชันความปลอดภัยคัดกรองเฉพาะพนักงานในโดเมนบริษัท
def is_authorised_employee(email):
    if not email:
        return False
    return email.strip().lower().endswith(ALLOWED_COMPANY_DOMAIN.lower())


# ========================================================
# 🚪 GATEWAY: ดักตรวจสอบสิทธิ์การเข้าใช้งาน (Authentication Check)
# ========================================================

if not st.session_state.get("connected", False):
    # --- กรณีพนักงานยังไม่ได้เข้าสู่ระบบ หรือยังไม่ได้กดปุ่ม Login ---
    st.markdown("""
        <div style='text-align: center; padding: 3rem 1rem;'>
            <h1 style='color: #2e7d32; font-weight: 700; font-size: 2.5rem;'>🔒 Intanin BD Secure Gateway</h1>
            <p style='opacity: 0.7; font-size: 1.1rem; margin-bottom: 2rem;'>ระบบจัดการเอกสารและวิเคราะห์ข้อมูลภายในแผนก Business Development</p>
        </div>
    """, unsafe_allow_html=True)
    
    st.info("💡 ข้อแจ้งเตือนความปลอดภัย: ระบบนี้เป็นความลับขององค์กร อนุญาตให้เข้าถึงได้เฉพาะพนักงานที่ใช้บัญชีอีเมลของบริษัทเท่านั้น")
    
    # แสดงปุ่ม "Sign in with Google" ของทางสถาปัตยกรรม Google Cloud
    authenticator.login()

else:
    # --- กรณีพนักงานกดล็อกอินผ่าน Google เรียบร้อยแล้ว ---
    user_email = st.session_state.get("user_info", {}).get("email", "")
    
    # ทำการตรวจโดเมนเมลซ้ำอีกครั้งหลังบ้านเพื่อป้องกัน Account ทั่วไป (@gmail.com) หลุดเข้ามา
    if not is_authorised_employee(user_email):
        st.error(f"❌ สิทธิ์การเข้าใช้งานถูกปฏิเสธ: บัญชี {user_email} ไม่ใช่อีเมลภายใต้โดเมนบริษัท ({ALLOWED_COMPANY_DOMAIN})")
        st.warning("หากคุณเป็นพนักงาน กรุณากดออกจากระบบแล้วเลือกเข้าสู่ระบบใหม่อีกครั้งด้วยอีเมลของบริษัท")
        if st.button("🔄 เข้าสู่ระบบด้วยบัญชีอื่น"):
            authenticator.logout()
            
    else:
        # ========================================================
        # ✅ [PASSED] ผ่านการตรวจสอบความปลอดภัย เข้าสู่ระบบหลักเดิมของคุณ
        # ========================================================
        ...
        # แสดงรูปภาพโปรไฟล์พนักงานและชื่อที่ดึงมาจาก Google Account ของบริษัท
        user_info = st.session_state.get("user_info", {})
        if user_info.get("picture"):
            st.sidebar.image(user_info.get("picture"), width=60)
        st.sidebar.write(f"**พนักงาน:** {user_info.get('name', 'ไม่ระบุชื่อ')}")
        st.sidebar.caption(f"📧 {user_email}")
        
        # เพิ่มปุ่ม Logout ปลอดภัยที่แถบข้าง
