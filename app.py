...
st.set_page_config(page_title="DO System V8.7 Master Fix", layout="wide")

st.title("📦 ระบบจัดการขนส่ง (V8.7 Layout Perfection)")
st.markdown("---")
...
if uploaded_file:
    ...
    tab1, tab2 = st.tabs(["✂️ 1. แยกซัพพลายเออร์", "📄 2. ออกใบส่งสินค้า (A4 Master)"])
    ...
    # --- TAB 2: ออกใบ DO (A4 Fix ตามรูปต้นฉบับ) ---
    with tab2:
        df_clean = df_raw.dropna(subset=['รหัสสาขา']).copy()
        if st.button("🚀 สร้างใบส่งสินค้า V8.7"):
            ...
            with pd.ExcelWriter(output_do, engine='xlsxwriter') as writer:
                ...
                worksheet.set_column('D:D', 15)  # Labels (Do No / Ref Po)
                worksheet.set_column('E:E', 18)  # Data (Numbers / Dates)
                
                # Styles
                f_comp = workbook.add_format({'bold': True, 'font_size': 14})
                f_title = workbook.add_format({'bold': True, 'font_size': 24, 'align': 'center', 'valign': 'vcenter'})
                ...
                for store_code in df_clean['รหัสสาขา'].unique():
                    ...
                    # --- Footer Section (3 Equal Columns) ---
                    f_row = r_ptr + 1
            ...
            st.success("✅ แก้ไข Layout V8.7 ตามรูปต้นฉบับเรียบร้อยแล้ว!")
            st.download_button("📥 ดาวน์โหลด DO Master V8.7", output_do.getvalue(), "DO_Final_V8.7.xlsx")